   /*
 *  时间：2017.10.25
 *  作者：from-orig-author-周华,fixed-new by-尹鹏孝
 */
...
(function (window) {
    ...
    //按键ASCII值
    jz.keys = function (e) {
     return jz.event(e).keyCode || jz.event(e).which || jz.event(e).charCode
    };
    ...
    jz.remove = function (t, time) {
        ...
        if (typeof time === "number"){
            setTimeout(function () { re(); }, time * 1000)
        } else {
            re()
        }
    ...
    jz.hint = function (t, msg) {
        ...
        if (window.addEventListener) {
            window.addEventListener("resize", autoxy, false);
        }else {
            window.attachEvent("onresize", autoxy);
        }

        setTimeout(function(){
            lbl.style.display = input.val().length ? "none" : "block";
        }, 800);
    ...
    jz.random = function (len) {
        len = arguments.length ? len > 15 ? 15 : len : 4; return Math.random().toString().substr(2, len)
    };

    //格式化JSON
    jz.parseJSON = function (s) {

        return jz.OldIE() ? (new Function("return " + s))() : JSON.parse(s)
